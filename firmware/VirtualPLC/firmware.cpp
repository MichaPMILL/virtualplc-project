// VirtualPLC CPU firmware: CPU, links with the Studio (USB serial, Wi-Fi), called from the
// sketch (VirtualPLC.ino). Kept out of the .ino so that the Arduino preprocessor does not
// generate prototypes for it.
#include "firmware.h"
...
struct Link {
    vplc::FrameParser parser;
    vplc::Session session;
};
Link serialLink;
