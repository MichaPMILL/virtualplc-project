...
interface HostBridge {
  ...
  openFiles(kind?: 'iodd' | 'gsdml' | 'eds' | 'gsd'): Promise<OpenedFile[]>;
  ...
  return {
    ...
    openFiles(kind) {
      return new Promise((resolve) => {
        ...
        input.type = 'file';
        input.multiple = true;
        input.accept = kind === 'eds' ? '.eds,.EDS' : kind === 'gsd' ? '.gsd,.gse,.gsg,.gsf,.gsi,.gsp,.gss,.GSD' : kind ? '.xml' : '.scl,.db,.udt,.xlsx,.xml,.txt';
