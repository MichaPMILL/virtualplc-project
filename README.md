...
- [`examples/carton-closer`](examples/carton-closer) — carton closing machine with pneumatic
  cylinders, programmed with **object orientation** (interface `ICylinder`, derived cylinders,
  sequence working on interface references);
- [`examples/traffic-lights`](examples/traffic-lights) — crossroads traffic lights with
  pedestrian call, vehicle detection and night mode, **without** object orientation.

Each one has its SCL sources and a ready-to-open Studio project (`project/`, regenerated by
`node examples/make-projects.ts`); both are tested in the VM (`sdk/test/examples.test.ts`).
